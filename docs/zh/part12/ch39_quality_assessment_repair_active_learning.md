# 第39章：数据质量评估、修复算法与主动学习

如果说 Ch38 解决的是“数据集怎么被描述成资产”，那么 Ch39 解决的就是“资产质量如何被持续提升”。很多团队在谈数据质量时，仍然停留在非常粗糙的层面：脏不脏、全不全、准不准。这样的说法对开会有用，对工程无用。因为一旦进入真实流水线，所谓“质量问题”并不是单个概念，而是一组相互关联的算法任务：质量评分、缺陷检测、错误归因、修复建议、规则学习、样本选择和人机闭环。
...
![图39-1：质量工程四层闭环](../../images/part12/chk_01_quality_four_layer_loop.png)
*图39-1 数据质量工程不是一次过滤，而是评分、检测、修复与采样的连续闭环。*
...
![图39-2：质量评分的三维坐标](../../images/part12/chk_02_quality_scoring_axes.png)
*图39-2 高质量不等于高价值，质量评分至少要同时考虑正确性、适配性与学习价值。*
...
![图39-4：六类数据的质量评分器组合](../../images/part12/chk_04_scorers_by_dataset.png)
*图39-4 不同数据类型需要不同评分器组合，单一总分会掩盖真正的质量矛盾。*
...
## 39.4 修复建议：规则、统计、表示学习与 LLM-as-Judge 的分工
...
![图39-3：质量修复分层决策树](../../images/part12/chk_03_repair_decision_tree.png)
*图39-3 规则、统计、表示学习与 Judge 不应互相替代，而应形成分层修复体系。*

### 39.4.1 质量修复流水线应该如何排布
...
![图39-5：质量评分到主动学习的闭环队列图](../../images/part12/chk_05_scoring_to_active_learning_queue.png)
*图39-5 建议突出 cheap filters、semantic triage、repair execution、post-repair evaluation 四段。*

![图39-6：不同缺陷类型的检测器与修复器对应表](../../images/part12/chk_06_detector_repairer_matrix.png)
*图39-6 可用表格展示逻辑缺陷、几何缺陷、行为缺陷、风格缺陷各自的处理链。*

![图39-7：主动学习优先级示意散点图](../../images/part12/chk_07_active_learning_priority_scatter.png)
*图39-7 横轴可设修复成本，纵轴可设修复收益，颜色表示业务风险等级。*

## 39.5 主动学习：有限人力应该优先标什么
