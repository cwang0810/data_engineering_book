...
![图41-1：多模态解析、RAG 与 Agent 数据的统一视角](../../images/part12/chm_01_three_data_families.png)
*图41-1 三类数据家族共享“证据链”思想，但监督重点各不相同。*

## 41.1 文档解析数据集：不仅是 OCR，更是结构与逻辑
...
![图41-2：文档理解数据的三层 schema](../../images/part12/chm_02_document_schema.png)
*图41-2 页面层、结构层与逻辑层共同构成文档理解数据的工程最小单元。*

### 41.1.1 文档解析数据集需要回答的五个问题
...
### 41.2.2 哪些图表最能帮助理解复合信息图推理

对于图表理解这一类任务，配图的价值并不在于增加版面丰富度，而在于帮助读者看清问题究竟依赖哪些证据区域。相比只给出抽象流程图，更值得优先保留的是那些能够直接展示“区域关系”和“证据类型”的图表：

![图41-3：复合信息图的区域依赖示意图](../../images/part12/chm_03_multichart_region_dependencies.png)
*图41-3 建议标出子图区域、图例区域、时间轴区域与问题链路。*

![表41-1：图表问题类型与所需证据类型对照表](../../images/part12/chm_04_chart_question_evidence_table.png)
*表41-1 可对照读数、比较、聚合、趋势、条件推理与不可回答问题。*

这两类图表最大的价值不在装饰，而在于帮助读者直观看到：为什么同样是“问图表问题”，有的题只需要单区读数，有的题必须跨区域拼接证据。

### 41.2.3 区域级证据标注为什么不能省
...
![图41-4：RAG evidence card 结构](../../images/part12/chm_03_rag_evidence_card.png)
*图41-3 高质量 RAG 数据的最小单元不是 Q/A，而是 Query、Answer 与 Evidence Bundle 的联立结构。*

### 41.3.1 推荐的 RAG Evidence Card 字段
...
### 41.3.2 RAG 数据集为什么必须显式记录 freshness 与 access level
...
### 41.3.5 Evidence Bundle 也需要版本管理
...
### 41.4.1 Agent Trajectory Schema 建议
...
![图41-5：Agent 轨迹数据的状态转移图](../../images/part12/chm_04_agent_trajectory_schema.png)
*图41-4 Agent 轨迹不是对话文本，而是带状态与观测回流的行动序列。*

### 41.4.2 为什么恢复样本是 Agent 数据最容易缺失的部分
...
这样的设计尤其适合 Ch42 继续展开，因为它天然连接到实验设计、切片归因和策略回写。

### 41.5.1 一张跨任务统一评测表应该如何组织
...
这张表很适合作为本章中段的“横向总览”，也方便后续在 Ch42 中承接实验矩阵。

### 41.5.2 跨任务统一基线不应该怎样搭
...
### 41.5.3 把多模态、RAG 和 Agent 写成同一篇的真正理由
...
### 41.5.5 多模态、RAG 与 Agent 的共享失败面板应该怎么建
...
这样的共享面板有两个好处。第一，它帮助团队把三个看似不同的任务重新对齐到同一套工程语言。第二，它为后续的 Ch42 和 Ch43 准备好了实验归因与榜单呈现的共同接口。换句话说，本章不是在并列介绍三类热门任务，而是在建立它们可以被放到同一个实验与治理体系里的原因。

### 41.5.6 为什么统一评测表里必须保留“不可恢复”与“不可回答”
