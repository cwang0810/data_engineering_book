...
![图1-1：数据质量对比图](../images/第1章/图1-1_数据质量对比.png)
...
![图1-2：LLM数据生命周期](../images/第1章/图1-2_LLM数据生命周期.png)
...
![图1-3：数据漏斗模型](../images/第1章/图1-3_数据漏斗模型.png)
...
![图1-4：多模态处理复杂度](../images/第1章/图1-4_多模态处理复杂度.png)
...
考虑一个实际场景：处理 10TB 的原始网页数据。假设平均每条数据需要经过 HTML 解析、文本提取、语言识别、质量评分、去重检查等五个步骤。每个步骤单条处理耗时 100 毫秒（已经是相当快的速度）。那么总处理时间约为：10TB ÷ 10KB/条 × 0.5 秒/条 ≈ 150 万 GPU 小时。按照云服务商的定价，这可能意味着数十万美元的成本——与模型训练本身的费用相当。
...
![图1-5：数据质量与训练效率](../images/第1章/图1-5_数据质量与训练效率.png)
...
![图1-6：知识结构思维导图](../images/第1章/图1-6_知识结构思维导图.png)
